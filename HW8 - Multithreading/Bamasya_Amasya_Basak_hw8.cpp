...
void start(int tid, int rid) {
	...
	if (teams[tid].currentRacer !=  teams[tid].count - 1) //unless it's the last runner
	{
		vecMutex.lock();
		teams[tid].currentRacer += 1; //next racer
		Runner nextrunner(teams[tid].currentRacer, tid);
		teams[tid].teamList.push_back(nextrunner); //put the runner to the team vector
		vecMutex.unlock();
		int runnerid = rid + 1;
		if (teams[tid].teamList[runnerid].t->joinable()) 
		{
			teams[tid].teamList[runnerid].t->join();
		}
...
void printRanking() {
	...
	{
		if (teams[i].rank == rank) //if that team is at that rank
		{
			cout << "[" << rank << "]" << " - " << "Team " << i << endl;
			rank++;
			i = -1; //to start the for loop from the beginning
		}
	}
	print.unlock();
}
